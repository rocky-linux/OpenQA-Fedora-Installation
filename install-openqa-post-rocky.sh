...
if [[ ! -f /var/lib/openqa/share/factory/iso/fixed/CHECKSUM ]]; then
  ...
  sudo curl -C - -O download.rockylinux.org/pub/rocky/8/isos/x86_64/Rocky-8.6-x86_64-boot.iso
  sudo curl -C - -O download.rockylinux.org/pub/rocky/8/isos/x86_64/Rocky-8.6-x86_64-minimal.iso
  sudo curl -C - -O download.rockylinux.org/pub/rocky/8/isos/x86_64/Rocky-8.6-x86_64-dvd1.iso
...
sudo openqa-cli api -X POST isos \
  ISO=Rocky-8.6-x86_64-minimal.iso \
  ARCH=x86_64 \
  DISTRI=rocky \
  FLAVOR=minimal-iso \
  VERSION=8.6 \
